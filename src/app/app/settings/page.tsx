...
export default function SettingsPage() {
  ...
  return <div className="min-h-screen bg-cream pb-28 text-ink">
    ...
    <main className="mx-auto max-w-xl px-4 py-6 space-y-7">
      <section><p className="text-xs font-bold uppercase tracking-[0.16em] text-ink-faint">Profile</p><div className="mt-2 rounded-2xl border border-line bg-card p-4"><div className="flex items-start justify-between gap-4"><div><h2 className="font-bold">Pause your profile</h2><p className="mt-1 text-sm leading-relaxed text-ink-soft">Pausing hides you from new Discover profiles. You can still chat with current matches.</p></div><button type="button" disabled={saving} onClick={() => save(!profile.is_active)} className={`relative mt-1 h-8 w-14 rounded-full transition-colors ${profile.is_active ? "bg-brand" : "bg-line-strong"}`} aria-label="Pause profile"><span className={`absolute top-1 h-6 w-6 rounded-full bg-white shadow transition-transform ${profile.is_active ? "translate-x-7" : "translate-x-1"}`} /></button></div></div></section>
